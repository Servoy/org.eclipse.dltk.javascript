...
class TokenStream {
    ...
    int peekToken() throws IOException {
        ...
        while (tt == Token.EOL || tt == Token.COMMENT) {
            if (tt == Token.EOL) {
                ...
                if (parser.compilerEnv.isRecordingComments()) {
                    String comment = getAndResetCurrentComment();
    ...
    boolean matchToken(int toMatch, boolean ignoreComment) throws IOException {
        int tt = peekToken();
    ...
    private void recordComment(int lineno, String comment) {
        ...
        Comment commentNode = commentType == Token.CommentType.LINE? new SingleLineComment(): new MultiLineComment();
        commentNode.setText(comment);
        commentNode.setStart(tokenBeg);
        commentNode.setEnd(getTokenLength());
        if (commentType == Token.CommentType.JSDOC
                && parser.compilerEnv.isRecordingLocalJsDocComments()) {
        	currentJsDocComment =new MultiLineComment();
            commentNode.setStart(tokenBeg);
            commentNode.setEnd(getTokenLength());
