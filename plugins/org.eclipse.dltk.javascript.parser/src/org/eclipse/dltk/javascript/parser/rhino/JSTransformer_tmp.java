...
public class JSTransformer_tmp {
...
//	private final int[] tokenOffsets;
	...
	public JSTransformer_tmp(Parser parser) {
		this(NodeTransformerManager.NO_TRANSFORMERS, parser, false);
	}

	public JSTransformer_tmp(Parser parser, boolean ignoreUnknown) {
		this(NodeTransformerManager.NO_TRANSFORMERS, parser, ignoreUnknown);
	}

	public JSTransformer_tmp(NodeTransformer[] transformers, Parser parser,
...
//		tokenOffsets = prepareOffsetMap(tokens);
	...
	protected final ASTNode visitNode() {
		ASTNode accept = visit();
//		if (accept == null) {
...
//		}
	...
	private ASTNode internalVisit() throws IOException {
...
//		case Token.OBJECT:
//			return visitObjectInitializer();
//
//		case Token.NAMEDVALUE:
//			return visitPropertyInitializer();
//
//		case Token.FOREACH:
//			return visitForEachInStatement();
...
//
//		case Token.XML_LITERAL:
//			return visitXmlLiteral();
//
//		case Token.DEFAULT_XML_NAMESPACE:
//			return visitNamespace();
//
//		case Token.XmlAttribute:
//			return visitXmlAttribute();

//		case Token.ALLCHILDREN:
//			return visitGetAllChildren();
//
//		case Token.LOCALNAME:
//			return visitGetLocalName();
//
//		case Token.HexIntegerLiteral:
//			return visitHexIntegerLiteral();
//
//		case Token.OctalIntegerLiteral:
//			return visitOctalIntegerLiteral();
	...
	public Script transformScript() throws IOException {
		final Script script = new Script();
		scope = new SymbolTable(script);

        int pos = 0;
        int baseLineno = parser.getLineNo(); // line number where source starts
        int end = pos; // in case source is empty

        try {
            for (; ; ) {
                int tt = parser.peekToken();
                if (tt <= Token.EOF) {
                    break;
                }
...
//                    }
                    ...
                    n = parser.scannedComments.get(parser.scannedComments.size() - 1);
                    parser.consumeToken();
                } else {
                    n = transformStatementNode(script);
...
//                    }
                ...
                else if (n instanceof JSDeclaration) {
                	script.addDeclaration((JSDeclaration) n);
                }
                if (n instanceof JSNode)
                  ((JSNode)n).setParent(script);
            }
        } catch (StackOverflowError ex) {
            String msg = "To much recusion in script";
            if (!parser.compilerEnv.isIdeMode())
                throw Context.reportRuntimeError(msg, parser.sourceURI, parser.getLineNo(), null, 0);
        } finally {
//            inUseStrictDirective = savedStrictMode;
        }

        if (parser.syntaxErrorCount != 0) {
            String msg = String.valueOf(parser.syntaxErrorCount);
            msg = "Syntax Errors Found";
            if (!parser.compilerEnv.isIdeMode())
                throw new RuntimeException(msg);
        }

        // add comments to root in lexical order
        if (parser.scannedComments != null) {
            // If we find a comment beyond end of our last statement or
            // function, extend the root bounds to the end of that comment.
            int last = parser.scannedComments.size() - 1;
            end = Math.max(end, parser.scannedComments.get(last).end());
            for (Comment c : parser.scannedComments) {
                script.addComment(c);
            }
        }
...
//	}
	...
	private ASTNode transformNode(JSNode parent) {
//		if (node == null) {
//			if (ignoreUnknown) {
...
//	}
	...
	private Statement transformStatementNode(JSNode parent) {

		ASTNode expression = transformNode( parent);
	...
	protected ASTNode visit() {
		final ASTNode node = internalVisit();
	...
	protected ASTNode visitUnknown() {
		if (ignoreUnknown) {
			return createErrorExpression();
		}
		throw new UnsupportedOperationException("Unknown token "
				+ Token.tokenNames[node.getType()] + " (" + node.getText()
				+ ")");
	}

	private ErrorExpression createErrorExpression() {
		if (node != null) {
	...
	protected ASTNode visitBinaryOperation() {
		if (node.getType() == Token.MUL) {
			switch (node.getChildCount()) {
			case 0:
				return visitAsterisk();
			case 1:
				// HACK
	...
	private Keyword createKeyword(String text) {
		final Keyword keyword = new Keyword(text);
		setRangeByToken(keyword, parser.getTokenStartIndex());
		return keyword;
	}

	protected ASTNode visitBreak() {
		BreakStatement statement = new BreakStatement(getParent());
		statement.setBreakKeyword(createKeyword(Keywords.BREAK));
	...
	protected ASTNode visitDecimalLiteral() {
	...
	protected ASTNode visitDefault() {
	...
	protected ASTNode visitExpression() {
		if (node.getChildCount() > 0)
	...
	private ASTNode visitForStatement() {
	...
	private ASTNode visitForInStatement() {
	...
	protected ASTNode visitIdentifier() {
	...
	protected ASTNode visitStringLiteral() {
	...
	protected ASTNode visitUnaryOperation() {
	...
	protected ASTNode visitContinue() {
	...
	protected ASTNode visitVarDeclaration() {
	...
	protected ASTNode visitObjectInitializer() {
	...
	private PropertyInitializer buildPropertyInitializer() {
	...
	protected ASTNode visitPropertyInitializer() {
		final PropertyInitializer initializer = buildPropertyInitializer();
	...
	protected ASTNode visitForEachInStatement() {
	...
	private static int getRealTokenStopIndex() {
	...
	protected ASTNode visitByField() {
	...
	protected ASTNode visitDoWhile() {
	...
	protected ASTNode visitConditional() {
	...
	protected ASTNode visitParenthesizedExpression() {
	...
	protected ASTNode visitFinally() {
	...
	protected ASTNode visitByIndex() {
	...
	protected ASTNode visitCommaExpression() {
	...
	protected ASTNode visitLabelled() {
	...
	protected ASTNode visitBooleanLiteral() {

		BooleanLiteral bool = new BooleanLiteral(getParent(),
				node.getType() == Token.TRUE);
	...
	protected ASTNode visitXmlLiteral() {
	...
	protected ASTNode visitNamespace() {
	...
	protected ASTNode visitXmlAttribute() {
	...
	protected ASTNode visitAsterisk() {
	...
	protected ASTNode visitGetAllChildren() {
	...
	protected ASTNode visitGetLocalName() {
	...
	protected ASTNode visitHexIntegerLiteral() {
		DecimalLiteral number = new DecimalLiteral(getParent());
		number.setText(intern(node.getText()));
		number.setStart(getTokenOffset(node.getTokenStartIndex()));
	...
	protected ASTNode visitOctalIntegerLiteral() {
		DecimalLiteral number = new DecimalLiteral(getParent());
		number.setText(intern(node.getText()));
		number.setStart(getTokenOffset(node.getTokenStartIndex()));
		number.setEnd(getTokenOffset(node.getTokenStopIndex() + 1));
	...
	protected ASTNode visitYield() {
		YieldOperator expression = new YieldOperator(getParent());

		expression.setYieldKeyword(createKeyword(Keywords.YIELD));
	...
	protected ASTNode visitEmptyStatement() {
